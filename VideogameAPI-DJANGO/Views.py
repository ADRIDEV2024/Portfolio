...
@api_view(['GET'])
def videogame_list_created(request):
    games = Videogame.objects.filter(published=True)
        
    if request.method == 'GET': 
        s_serializer = VideogameSerializer(videogames, many=True)
        return JsonResponse(videogame_serializer.data, safe=True)
